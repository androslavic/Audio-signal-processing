...
#define SLAVE_ADDRESS 0x18 // the slave address (example)
#define ADDRESS		0x18 // 0011000 tlv datasheet str 41
...
I2C_start(I2C1, SLAVE_ADDRESS<<1, I2C_Direction_Receiver); // start a transmission in Master receiver mode
	...
	for (i=0;i<4;i++){
		received_data[i] = I2C_read_ack(I2C1); // read one byte and request another byte
		
		sprintf(str, "%d", received_data[i]);		
		USART_puts(USART6, str);
 		USART_puts(USART6, "\r\n");	
	}

		received_data[4] = I2C_read_nack(I2C1); // read one byte and don't request another byte, stop transmission
	sprintf(str, "%d", received_data[40]);		
		USART_puts(USART6, str);
 		USART_puts(USART6, "\r\n");	
